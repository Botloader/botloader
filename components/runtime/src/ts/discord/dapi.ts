...
export interface CreateAttachment {
    ...
    /**
     * The placeholder ID for the attachment when creating it
     * This will be autogenerated by botloader if not provided
     * 
...
export function toOpMessageFields(fields: CreateMessageFields): Internal.OpCreateMessageFields {
    ...
    const userProvidedIds = (fields.attachments ?? []).map(v => v.id).filter(v => v !== undefined) as number[]
    let idIncrementer = 0
    // Provides automatic id generation for attachments, avoiding collisions with user provided ids
    const nextAutoId = () => {
        while (true) {
            idIncrementer++

            let next = idIncrementer
            if (userProvidedIds.includes(next)) {
                continue
            }
