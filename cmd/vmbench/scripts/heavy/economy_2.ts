...
script.createCommand(
    Commands.slashCommand("v2-balance", "check your wallet and bank balance")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("v2-daily", "claim your daily coins")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("v2-work", "work a random job for coins")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("v2-deposit", "move coins from wallet to bank")
...
script.createCommand(
    Commands.slashCommand("v2-withdraw", "move coins from bank to wallet")
...
script.createCommand(
    Commands.slashCommand("v2-pay", "send coins to another user")
...
script.createCommand(
    Commands.slashCommand("v2-shop", "browse the item shop")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("v2-buy", "buy an item from the shop")
...
script.createCommand(
    Commands.slashCommand("v2-inventory", "see what you own")
        .build(async (ctx) => {
            const inv = (await inventories.get(ctx.member.user.id))?.value ?? {};
...
script.createCommand(
    Commands.slashCommand("v2-rich", "leaderboard of the richest users")
        .build(async (ctx) => {
