...
script.createCommand(
    Commands.slashCommand("v2-giveaway-start", "start a giveaway")
...
script.createCommand(
    Commands.slashCommand("v2-giveaway-end", "end a giveaway early")
...
script.createCommand(
    Commands.slashCommand("v2-giveaway-reroll", "pick new winners for an ended giveaway")
        .addOptionString("id", "giveaway id")
        .build(async (ctx, args) => {
            const entry = await giveaways.get(args.id);
            if (!entry || !entry.value.ended) {
...
script.createCommand(
    Commands.slashCommand("v2-giveaway-wins", "see who has won the most giveaways")
        .build(async (ctx) => {
...
script.onInterval("v2_giveaway_ticker", 1, async () => {
    const all = await giveaways.list({ limit: 50 });
