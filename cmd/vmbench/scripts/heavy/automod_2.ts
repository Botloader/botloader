...
script.createCommand(
    Commands.slashCommand("v2-automod-list", "list automod rules and their status")
        .build(async (ctx) => {
            const rules = (await rulesVar.get())?.value ?? DEFAULT_RULES;
...
script.createCommand(
    Commands.slashCommand("v2-automod-toggle", "enable or disable an automod rule")
...
script.createCommand(
    Commands.slashCommand("v2-automod-threshold", "change a rule's threshold")
        .addOptionString("rule", "rule key, see /automod-list")
...
script.createCommand(
    Commands.slashCommand("v2-automod-violations", "check a user's violation count")
...
script.onInterval("v2_automod_cleanup", 15, () => {
