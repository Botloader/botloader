...
script.createCommand(
    Commands.slashCommand("ticket-open", "open a support ticket")
...
script.createCommand(
    Commands.slashCommand("ticket-claim", "claim a ticket as a staff member")
...
script.createCommand(
    Commands.slashCommand("ticket-close", "close a ticket")
        .addOptionInteger("id", "ticket number")
...
script.createCommand(
    Commands.slashCommand("ticket-info", "show details about a ticket")
        .addOptionInteger("id", "ticket number")
        .build(async (ctx, args) => {
            const entry = await tickets.get(`${args.id}`);
            if (!entry) {
...
script.createCommand(
    Commands.slashCommand("ticket-list", "list open tickets")
...
script.onInterval("ticket_auto_close", "0 * * * *", async () => {
    const all = await tickets.list({ limit: 100 });
