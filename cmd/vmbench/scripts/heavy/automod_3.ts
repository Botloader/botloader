...
script.createCommand(
    Commands.slashCommand("v3-automod-list", "list automod rules and their status")
        .build(async (ctx) => {
            const rules = (await rulesVar.get())?.value ?? DEFAULT_RULES;
...
script.createCommand(
    Commands.slashCommand("v3-automod-toggle", "enable or disable an automod rule")
...
script.createCommand(
    Commands.slashCommand("v3-automod-threshold", "change a rule's threshold")
        .addOptionString("rule", "rule key, see /automod-list")
...
script.createCommand(
    Commands.slashCommand("v3-automod-violations", "check a user's violation count")
...
script.onInterval("v3_automod_cleanup", 15, () => {
