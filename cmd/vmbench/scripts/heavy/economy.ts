...
script.createCommand(
    Commands.slashCommand("balance", "check your wallet and bank balance")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("daily", "claim your daily coins")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("work", "work a random job for coins")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("deposit", "move coins from wallet to bank")
...
script.createCommand(
    Commands.slashCommand("withdraw", "move coins from bank to wallet")
...
script.createCommand(
    Commands.slashCommand("pay", "send coins to another user")
...
script.createCommand(
    Commands.slashCommand("shop", "browse the item shop")
        .build(async (ctx) => {
...
script.createCommand(
    Commands.slashCommand("buy", "buy an item from the shop")
...
script.createCommand(
    Commands.slashCommand("inventory", "see what you own")
        .build(async (ctx) => {
            const inv = (await inventories.get(ctx.member.user.id))?.value ?? {};
...
script.createCommand(
    Commands.slashCommand("rich", "leaderboard of the richest users")
        .build(async (ctx) => {
